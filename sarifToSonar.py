    ...
    for run in sarif_data.get("runs", []):
        for result in run.get("results", []):
            for location in result.get("locations", []):
                ...
                # Extract the resource identifier from properties (if available)
                resource = result.get("properties", {}).get("resource", "")
                if resource:
                    # Assume the format is 'resource_type.resource_name', e.g., 'aws_instance.my_ec2'
                ...
                issue_data = {
                    ...
                    "tags": [resource_type]  # Add the resource type identifier as a tag
                }
